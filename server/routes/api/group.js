...
router.post('/join/:group_id',passport.authenticate('jwt',{session: false}),(req,res) => {
    ...
    Group.findOne({_id: req.params.group_id})
         .then(group => {
             if(group) {
                ...
                if(group.users.filter(user => user.user.toString() === req.user.id).length > 0){
                    res.json({msg: 'Already added to the group'})
                }

                group.users.unshift({user: req.user.id});
                group.save()
                     .then(group => res.json({msg: 'Added to group successfully',group:group}))
                     .catch()
...
router.get('/messages/:group_id',passport.authenticate('jwt',{session: false}),(req,res) => {
    Group.findOne({_id: req.params.group_id})
         .then(group => {
             if(!group){
...
router.post('/addMessage/:group_id',passport.authenticate('jwt',{session: false}),(req,res) => {
              ...
              if(!group) {
                  ...
                  newMessage.save()
                            .then(message => {
                                ...
                                Group.findOneAndUpdate({_id: group._id},{$set: group},{new: true})
                                     .then(group => res.status(200).json({status:'Message added successfully',group}))
                                     .catch(err => {throw err});   
                            })
                            .catch(err => {throw err});
              }
